 ...
 *	\param		comp		(keytree_comp_node_cb)ツリー挿入用の比較関数
 *	\param		virt		(keytree_comp_node_cb)ツリー探索用の仮想比較ノード構築関数
 *	\return				(void)
 */
...
/**
 *	\fn			yourtree_ref_nth
 *	\brief		ツリー中の、N番目のノードを参照する
 *	\param		nth		(int)インデックス。マイナスで末尾からの数。
 *	\return			(nodetype_t*)取得できたノードのポインタ。ない場合はNULL。
 */
#define	KEYTREE_REF_NTH_(yourtree)		yourtree##_ref_nth

/**
 *	\fn			yourtree_get_belong
...
/**
 *	\fn			yourtree_find_eq_node
 *	\brief		ツリーから該当する値「同等」の最小ノードを探る
 *	\param		self		(yourtree_t*)取り扱うツリー
 *	\param		node		(node_type_t*)比較対象の構造体のポインタ
 *	\return				(node_type_t*)該当するうち最小に位置するノード。ない場合はNULL
 */
#define	KEYTREE_FIND_EQ_NODE_(yourtree)		yourtree##_find_eq_node

/**
 *	\fn			yourtree_find_eq_end_node
 *	\brief		ツリーから該当する値「同等」の最大ノードを探る
 *	\param		self		(yourtree_t*)取り扱うツリー
 *	\param		node		(node_type_t*)比較対象の構造体のポインタ
 *	\return				(node_type_t*)該当するうち最小に位置するノード。ない場合はNULL
 */
#define	KEYTREE_FIND_EQ_END_NODE_(yourtree)	yourtree##_find_eq_end_node

/**
 *	\fn			yourtree_find_ge_node
 ...
 *	\return				(node_type_t*)該当するうち最小に位置するノード。ない場合はNULL
 */
#define	KEYTREE_FIND_EQ_END_VALUE_(yourtree)	yourtree##_find_eq_end_value

/**
 *	\fn			yourtree_find_ge_value
 *	\brief		ツリーから該当する値「以上」の最小ノードを探る
 *	\param		self		(yourtree_t*)取り扱うツリー
 *	\param		value		(void*)キーとする値のポインタ
 *	\param		value_len	(size_t)valueポインタの長さ。例えば文字列長
...
/**
 *	\fn			yourtree_init_iterator_ranged
 *	\brief		ツリーのイテレータを、始端、開始点、終端の順に指定して初期化する
 *	\param		self		(yourtree_t*)取り扱うツリー
 *	\param		iterator	(yourtree_iterator_t*)イテレータ
 *	\param		head		(node_type_t*)始端。NULLでself->head相当。
 *	\param		from		(node_type_t*)開始位置の指定。NULLでhead/tailから開始。
 *	\param		tail		(node_type_t*)終端。NULLでself->tail相当。
 *	\return				(int)0で成功、失敗でマイナス。
 *	\remarks				head,from,tailはselfに所属していなければならない。さもなきゃ失敗。
 *	\remarks				head <= from <= tailを守ろう。
 */
...
/**
 *	\fn			yourtree_has_node
 *	\brief		ノードをツリーが持っているかをチェックする
 *	\param		node	(nodetype_t*)対象ノード
 *	\return			(int)所属してる場合で1、それ以外で0
 */
#define	KEYTREE_HAS_NODE_(yourlist)	yourtree##_has_node
